...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + pad, check_bb.Min.y + pad), ImVec2(check_bb.Max.x - pad, check_bb.Max.y - pad), ImColor(255, 255, 182, int(t * 205)), 2);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 9), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 3 - 8), ImColor(255, 255, 182, 255), style.GrabRounding);
        window->DrawList->AddCircleFilled(ImVec2(grab_bb.Max.x - 5, grab_bb.Max.y - 5 - 8), 5, ImColor(255, 255, 202, 255), 24);
        if (IsItemActive() || IsItemHovered())
            window->DrawList->AddCircleFilled(ImVec2(grab_bb.Max.x - 5, grab_bb.Max.y - 5 - 8), 7, ImColor(255, 255, 202, 55), 24);
    }
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
